...
public class Application {
    public static void main(String[] args) throws InvocationTargetException, InstantiationException, IllegalAccessException {
         ...
         *   스프링 프레임워크, 마이바티스, 하이버네이트, jackson 등의 라이블러리에서 사용된다.
        ...
        * */
        ...
        for (Constructor constructor : constructors) {
            System.out.println("nale: " + constructor.getName());
...
//        System.out.println(constructors[0]);
        ...
        for (Method method : methods) {
            System.out.println(Modifier.toString(method.getModifiers()) + " " + method.getReturnType().getSimpleName()
             + " " + method.getName());
            if ("getBalace".equals(method.getName())) {
                getBalanceMethod = method;
            }
        }

        try{
            /* 설명. nonstatic 메소드만 따로 실행하고 싶을때는 해당 메소드가 실행되기 위한 객체를 넘겨서 실행할 수 있다.*/
