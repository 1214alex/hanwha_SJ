...
//    @Before("execution(* com.ohgiraffers.section01.aop.*Service.*(..))")
    ...
    /* 설명. Pointcut에 해당한느 위치가 담긴 메소드가 같은 클래스에 있으면 클래스 소속 생략 가능*/
    /* 설명. returning에 쓰인 이름이 반환값이 넘어올 때의 이름이고 매개변수와 일치해야 한다.(result)*/
    @AfterReturning(pointcut = "LoggingAspect.loggingAspect()", returning = "result")
     ...
     *  하지만 타켓 메소드를 실행하는 proceed() 메소드를 잊는 경우가 자주 발생하기 때문에 사용 시
    ...
    * */
    ...
    public Object logAround(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        /* 설명. 실행된 타켓 메소드를 반환(다른 어드바이스 다시 실행될 수 있다.) */
        return result;
    }
}
