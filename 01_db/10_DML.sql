...
-- 컬럼명 미작성시에는 실제 테이블의 컬럼 갯수만큼 컬럼 순서를 지켜 작성해야한다.
...
UPDATE tbl_menu
	SET category_code = 7 -- 여기서의 =은 대입 기호
 WHERE menu_code = 25;
...
-- mariadb는 기본적으로 autocommit 설정이 'on'으로 되어 있으므로 우리가 수동적으로 하기 위해 autocommit을 꺼주어야 한다.
SET autocommit = 1;
SET autocommit = ON;
