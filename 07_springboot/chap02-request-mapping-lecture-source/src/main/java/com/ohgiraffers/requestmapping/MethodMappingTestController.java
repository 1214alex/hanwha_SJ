...
//    @RequestMapping(value = "/menu/regist")   //  GET 요청 뿐 아니라 다른 요청도 받음
    ...
    public String registMenu(Model model) {     //NOTE Model은 SSR에서 쓰는 것으로 응답할 페이지의 재료가 된다.
        System.out.println("/menu/regist 경로의 GET 요청 받아보기");

        model.addAttribute("message", "신규 메뉴 등록용 핸들러 메소드 호출함...");

        /* 설명. 핸들러 메소드에서 변환한 String은 응답 값이 아닌 view(html 파일 이름)이 된다. */
        return "mappingResult";     //NOTE view의 이름
    }
